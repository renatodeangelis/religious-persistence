...
| `code/00-run-all.R` | Sources all numbered scripts 01–20 in dependency order. `robustness-add-health.R` requires restricted Add Health files and must be run manually |
...
| `code/06-movement-measures.R` | OM/SM/EM/SSM trends + diagonal exit probability (1−Pᵢᵢ) by origin class, for 5- and 10-year cohort bins → `output/figures/movement/` |
| `code/07-lambda2-bootstrap.R` | λ₂ trend with bootstrapped CIs (250 replicates) → `output/figures/lambda2_trend_10yr.png` |
| `code/08-age-standardization-explore.R` | **Exploratory**: age-standardized λ₂/mean-diagonal sensitivity, U-vs-S matrix grids → `output/figures/explore/` |
| `code/09-period-cohort-decomp-explore.R` | **Exploratory**: age-period-cohort decomposition grids and two-way effects → `output/figures/explore/apc/` |
...
| `code/16-robustness-attend12.R` | **Robustness**: transition matrices stratified by childhood attendance frequency (`attend12`); available in GSS waves 1991, 1998, 2008, 2018 only → `output/figures/attend12/`. Consumes `gss_clean.rds` |
| `code/17-robustness-demographics.R` | **Robustness**: matrices stratified by `sibs_group` (siblings of origin) and `childs_group` (respondent's own children); 5 cohort windows 1940–1989 → `output/figures/demographics/`. Consumes `gss_clean.rds` |
| `code/18-diff-figures-sex.R` | Difference matrix grid (P_male − P_female) + π₀/π* dot-plot comparison across cohorts → `output/figures/sex/` |
| `code/19-diff-figures-nonblack.R` | Difference matrix grid (P_nonblack − P_full) + π₀/π* dot-plot comparison across cohorts → `output/figures/nonblack/` |
