...
pi0_list_5    = list()
pistar_list_5 = list()
N_list_5      = list()   # raw counts feeding the rolling-window sensitivity below
...
pi0_list_10    = list()
pistar_list_10 = list()
N_list_10      = list()   # raw count matrices feeding the homogeneity test below
...
# Substantive band to highlight — illustrative; adjust to the cohorts of interest.

p_roll = ggplot(roll_results, aes(x = center, y = p_value,
                                  color = k_label, shape = k_label, group = k_label)) +
  geom_hline(yintercept = 0.05, linetype = "dashed", color = "grey30", linewidth = 0.5) +
  geom_vline(xintercept = focal, linetype = "dotted", color = "grey50", linewidth = 0.4) +
  ...
  labs(
    ...
    caption = paste("Each point is a joint homogeneity test (Anderson-Goodman 1957) on the k",
                    ...
                    "matrices, not\ncalendar span). Dashed line: p = 0.05. Shaded band and dotted",
                    "line mark focal cohorts (illustrative; set in code).")) +
  healy_theme +
