# ── 21 · ROBUSTNESS: SEVEN-STATE RELTRAD SCHEME ──────────────────────────────
# Estimates transition matrices using all seven RELTRAD categories — without
# collapsing black protestant into evangelical or jewish into other.
...
# ── STATE SPACE ───────────────────────────────────────────────────────────────
# `reltrad` and `reltrad16` are the uncollapsed 7-state character columns saved
# as-is in gss_clean.rds (set by reltrad_labels in 01; not mutated to _alt/_bp).

states_7 = sort(unique(c(data$reltrad, data$reltrad16)))
states_7  = states_7[!is.na(states_7)]

# Ordering: Catholic first; Protestant block (evangelical → black protestant →
# mainline); minority traditions (jewish → other); none last.
rel_level_order_7 = c("catholic", "evangelical", "black protestant",
                       "mainline", "jewish", "other", "none")

# Okabe-Ito colors: add amber (black protestant) and sky blue (jewish) to the
# existing 5-state palette. Both are unused in the main 5-state figures.
reltrad_colors_7 = c(
  "catholic"          = "#0072B2",
  "evangelical"       = "#D55E00",
...
# ── THIN-CELL CHECK ───────────────────────────────────────────────────────────
# Jewish (~2–3% of GSS) may fall below 30 as an origin state in early cohorts.
# Black Protestant (~8–10%) should be above 30 in all windows.
...
dir.create("output/figures/7state", recursive = TRUE, showWarnings = FALSE)

# ── FIGURE 1: TRANSITION MATRIX GRID ─────────────────────────────────────────
# 7×7 heatmap, 3 rows × 2 columns. Asterisk in strip label if any at-risk cell.
...
p_grid_7 = ggplot(grid_df_7, aes(current, origin, fill = p)) +
  ...
  labs(x = "Current religion (RELIG)", y = "Origin (RELIG16)",
       title = "Intergenerational transition matrices by birth cohort (7-state)",
...
ggsave("output/figures/7state/P_grid_7state_10yr.png", p_grid_7,
       width = 10, height = 14, dpi = 200)

# ── FIGURE 2: λ₂ TREND WITH BOOTSTRAPPED CIs ─────────────────────────────────
# Scalar computation identical to 07-lambda2-bootstrap.R; only input matrices differ.
...
cat("\nλ₂ estimates (7-state) with 95% bootstrap CIs (n_boot = 250, seed = 42):\n")
...
p_l2_7 = ggplot(boot_df_7, aes(x = cohort, y = l2, group = 1)) +
  ...
  labs(
    ...
    title = expression(lambda[2] ~ "(7-state) by birth cohort with 95% bootstrap CIs")
  ) +
  healy_theme
...
ggsave("output/figures/7state/pi_dist_grid_7state_10yr.png", p_dist_grid_7,
       width = 14, height = 5, dpi = 200)

cat("\nSaved all 7-state figures to output/figures/7state/\n")

# ── KEY ASSUMPTIONS ───────────────────────────────────────────────────────────
# - `reltrad` and `reltrad16` in gss_clean.rds carry the original 7-state values
#   unchanged; any filtering in script 01 applies equally to both, so no new
